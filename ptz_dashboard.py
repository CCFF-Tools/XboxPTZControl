...
for(const id of ['maxSpeed','deadzone','zoomSpeed','deckBrightness','deckEnabled'])$(id).oninput=markDirty;$('save').onclick=save;$('reload').onclick=()=>loadConfig(true);$('logs').onclick=logs;$('addCamera').onclick=()=>addCamera();$('discover').onclick=discover;refresh();logs();setInterval(refresh,5000);
</script></body></html>"""
