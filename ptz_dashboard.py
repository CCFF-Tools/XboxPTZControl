...
<section class="card"><h2>Tuning</h2><div class="controls"><label>Maximum speed<input id="maxSpeed" type="number" min="1" max="24"></label><label>Deadzone<input id="deadzone" type="number" min="0" max="0.5" step="0.01"></label><label>Zoom speed<input id="zoomSpeed" type="number" min="0" max="7"></label></div></section>
...
function renderConfig(config){$('cameras').replaceChildren(...config.cameras.map(cameraRow));$('maxSpeed').value=config.max_speed;$('deadzone').value=config.deadzone;$('zoomSpeed').value=config.zoom_speed;dirty=false}
function buildConfig(){return{cameras:[...$('cameras').children].map(cameraFromRow),max_speed:Number($('maxSpeed').value),deadzone:Number($('deadzone').value),zoom_speed:Number($('zoomSpeed').value)}}
function renderControllers(data){const items=[];if(data.state.controller?.connected)items.push('Active: '+data.state.controller.name+(data.state.controller.wireless?' (wireless)':''));for(const pad of data.controllers)items.push(pad.name);$('controller').replaceChildren(...(items.length?items:['No controller connected']).map(value=>text('div',value)))}
async function loadConfig(force=false){if(dirty&&!force)return;renderConfig(await api('/api/config'))}
async function refresh(){try{const data=await api('/api/status');const state=data.state;const uptime=data.uptime==null?'unknown':Math.floor(data.uptime/3600)+'h';$('status').replaceChildren(text('div',data.hostname+' • '+(state.stale?'offline/stale':'online'),state.stale?'bad':'ok'),text('div','Host uptime '+uptime+' • load '+data.load.map(v=>v.toFixed(2)).join(' / ')),text('div','Speed '+state.max_speed+' • deadzone '+state.deadzone+' • zoom '+state.zoom_speed));renderControllers(data);if(!$('discoverSubnet').value&&data.local_networks.length)$('discoverSubnet').value=data.local_networks[0];await loadConfig();if(!dirty){[...$('cameras').children].forEach((row,index)=>{const value=data.cameras[index]?.reachability||'unknown';const health=row.querySelector('.health');health.textContent='Automatic status: '+value;health.className='health '+(value==='reachable'?'ok':value==='unreachable'?'bad':'muted')})}$('msg').textContent=dirty?'Connected • unsaved changes':'Connected'}catch(error){$('msg').textContent='Authentication or service error: '+error.message}}
...
for(const id of ['maxSpeed','deadzone','zoomSpeed'])$(id).oninput=()=>dirty=true;$('save').onclick=save;$('reload').onclick=()=>loadConfig(true);$('logs').onclick=logs;$('addCamera').onclick=()=>addCamera();$('discover').onclick=discover;refresh();logs();setInterval(refresh,5000);
</script></body></html>"""
