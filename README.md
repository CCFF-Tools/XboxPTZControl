...
| `/dev/input/js0` exists but still waiting for joystick | Logs now show whether `/dev/input/js*` are readable (or why they fail to open). If you see “Joystick open failed”, fix permissions/udev and restart. Set `PTZPAD_DEBUG_INPUT=1` in the service environment to stream joystick input samples and VISCA actions while troubleshooting stuck input. |
